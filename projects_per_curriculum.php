...
if(isset($_GET['id'])){
    ...
    $qry = $conn->query("SELECT * FROM khoa where `status` = 1 and id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
    ...
    <div class="col-12">
        <div class="card card-outline card-primary shadow rounded-0">
            <div class="card-body rounded-0">
                <h2>Danh sách đề tài theo <?= isset($curriculum['ten']) ? $curriculum['ten'] : "" ?> </h2>
                <p><small><?= isset($curriculum['mota']) ? $curriculum['mota'] : "" ?></small></p>
                <hr class="bg-navy">
                <?php 
                ...
                $wherecid = " and makhoa = '{$id}' ";
                $students = $conn->query("SELECT * FROM `sinhvien` where id in (SELECT mssv FROM detai where `status` = 1 {$wherecid})");
                $student_arr = array_column($students->fetch_all(MYSQLI_ASSOC),'email','id');
                $count_all = $conn->query("SELECT * FROM detai where `status` = 1 {$wherecid}")->num_rows;    
                $pages = ceil($count_all/$limit);
                $detai = $conn->query("SELECT * FROM detai where `status` = 1 {$wherecid} order by unix_timestamp(date_created) desc {$paginate}");    
                ?>
